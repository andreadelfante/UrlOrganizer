...
class Experiment:
    ...
    def run(self, working_directory, clustering_algorithm, word2Vec_conf, vector_type):
        ...
        if (dim_link > 0 and dim_content > 0):
            combined_matrix = np.array(
                [np.concatenate((content_matrix[i], embedding_matrix[i])) for i in range(0, len(content_matrix))])
    ...
    def main(self, argv):
        if len(argv) != 4:
            ...
            print(
                'run Experiment.py <directory_path> <clustering algorithm (KMEANS or HDBSCAN)> <word2Vec conf (negative, h_softmax, None)> <vectors_type(link, content, combined)>')
            sys.exit(2)
        else:
            directory = argv[0]
...
if __name__ == "__main__":
    ...
    Experiment().main([direct, clustering, w2v_conf, vectors_conf])
