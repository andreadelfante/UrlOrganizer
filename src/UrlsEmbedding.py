...
class UrlsEmbedding:
    def __init__(self, filename, scaling=Scale.zscore):
        ...
        self.embeddings, self.urls = self.__read_embeddings(filename)
    ...
    def test(self, real_lables, learned_labels):
        print('homo compl   v-meas  ARI AMI')
        print('%.3f   %.3f   %.3f   %.3f   %.3f',
              metrics.homogeneity_score(real_lables, learned_labels),
              metrics.completeness_score(real_lables, learned_labels),
              metrics.v_measure_score(real_lables, learned_labels),
              metrics.adjusted_rand_score(real_lables, learned_labels),
              metrics.adjusted_mutual_info_score(real_lables, learned_labels),
              metrics.silhouette_score(self.normalized_embeddings, learned_labels,
                                       metric='euclidean'))
